  ...
  return (
    <>
      ...
      <div className="max-w-3xl space-y-5 px-6 py-8 lg:px-8">
        {notice && <p className="text-[12.5px] text-[#A0A0B0]">{notice}</p>}
        ...
        {proposed.length > 0 && (
          <div className="flex items-center justify-between rounded-xl border border-[#6C47FF]/30 bg-[#6C47FF]/8 px-4 py-3.5">
        ...
        {loading ? (
          ...
          <p className="py-8 text-center text-[12.5px] text-[#6E6E80]">
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-[#1E1E2E] bg-[#12121A] px-4 py-3">
      <span className="block text-[11px] uppercase tracking-[0.14em] text-[#6E6E80]">{label}</span>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <button onClick={onToggle} className="min-w-0 flex-1 text-left">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[11px] uppercase ${TIER_COLOUR[sequence.sequence_type] ?? "text-[#A0A0B0]"}`}>
              {sequence.sequence_type}
            </span>
          </div>
          <p className="mt-0.5 flex items-center gap-1.5 text-[11.5px] text-[#6E6E80]">
        ...
        <div className="ml-3 flex shrink-0 items-center gap-1.5">
          <span className="rounded-full border border-[#2A2A3A] px-2 py-0.5 text-[10.5px] text-[#A0A0B0]">
            {sequence.status}
          </span>
          {sequence.status === "proposed" && (
            <button onClick={onActivate} className="h-7 rounded-full bg-[#6C47FF] px-3 text-[11px] font-medium text-white">
              Activate
            </button>
          )}
          {sequence.status === "active" && (
            <button onClick={onPause} className="h-7 rounded-full border border-[#2A2A3A] px-3 text-[11px] text-[#A0A0B0]">
              Pause
            </button>
          )}
        </div>
      ...
      {expanded && (
        <div className="space-y-1.5 border-t border-[#1E1E2E] px-4 py-3">
          {sequence.touchpoints.map((touchpoint, index) => (
            <div key={index} className="flex items-start gap-3 rounded-lg bg-[#0D0D17] px-3 py-2">
              <span className="mt-0.5 shrink-0 text-[11px] tabular-nums text-[#6E6E80]">
                D+{touchpoint.dayOffset} {hourLabel(touchpoint.hour)}
              </span>
              <span className="min-w-0 flex-1">
                <span className="block text-[12px] text-white">{touchpoint.intent}</span>
                <span className="mt-0.5 block text-[11px] text-[#6E6E80]">
                  {touchpoint.channel}
