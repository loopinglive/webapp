  ...
  return (
    <main className="relative min-h-screen">
      ...
      <article className="mx-auto max-w-[760px] px-6 pb-20 pt-28">
        ...
        <h1 className="mt-3 text-[38px] font-semibold leading-[1.1] tracking-[-0.03em] text-ink">
          {title}
        </h1>
        <p className="mt-3 text-[13px] text-ink-faint">Last updated {updated}</p>

        <div className="mt-6 text-[15px] leading-relaxed text-ink-muted">{intro}</div>

        <nav
          aria-label="Contents"
          className="mt-10 rounded-2xl border border-hairline bg-surface p-5 print:hidden"
        >
        ...
        <div className="mt-12 space-y-10">
          {sections.map((section, index) => (
            <section key={section.id} id={section.id} className="scroll-mt-24">
              ...
              <div className="mt-3 space-y-3 text-[14.5px] leading-relaxed text-ink-muted [&_a]:text-accent [&_a:hover]:text-[#8A6BFF] [&_li]:ml-5 [&_li]:list-disc [&_strong]:text-[#D4D4DE]">
                {section.body}
              </div>
            </section>
          ))}
        ...
        <p className="mt-14 border-t border-hairline pt-6 text-[13.5px] text-ink-faint">
          Questions about this document? Email{" "}
          <a href="mailto:support@loopinglive.com" className="text-accent">
