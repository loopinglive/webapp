...
export function UpgradeWall({
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[980px] rounded-3xl border border-hairline bg-void p-6 shadow-2xl sm:p-10">
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full text-ink-faint transition-colors hover:bg-surface-2 hover:text-ink"
        >
          <X className="h-4 w-4" />
        </button>

        <div className="text-center">
          <span className="inline-grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-accent to-cyan">
            <Rocket className="h-5 w-5 text-ink" />
          </span>

          <h2
            id="upgrade-wall-title"
            className="mt-4 text-[26px] font-semibold tracking-[-0.025em] text-ink sm:text-[30px]"
          >
            You are ready to go live
          </h2>
          <p className="mx-auto mt-2 max-w-[46ch] text-[14.5px] leading-relaxed text-ink-muted">
