...
export function PaystackCheckout({ planSlug, label }: { planSlug: PlanSlug; label: string }) {
  ...
  async function start() {
    ...
    setError(null);
    try {
      const response = await fetch("/api/payments/local/paystack/initiate", {
        method: "POST",
      ...
      setError("Could not reach Paystack.");
      setPending(false);
    }
  }
