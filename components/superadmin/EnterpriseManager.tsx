...
export function EnterpriseManager() {
  ...
  return (
    <div className="space-y-8 px-6 py-6 lg:px-8">
      <section>
        <h2 className="text-[13px] font-semibold text-ink">New demo requests</h2>
        {leads.length === 0 ? (
          <p className="mt-2 text-[12.5px] text-ink-faint">Nothing waiting.</p>
        ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      className="h-7 rounded-md border border-hairline px-2.5 text-[11.5px] text-ink-muted hover:text-ink disabled:opacity-60"
                    >
                      Mark contacted
                    </button>
                    <button
                      ...
                      disabled={busy === lead.id}
                      className="h-7 rounded-md px-2.5 text-[11.5px] text-ink-faint hover:text-[#FF5A5A] disabled:opacity-60"
                    >
                      Dismiss
                    </button>
                  </div>
      ...
      <section className="rounded-2xl border border-hairline bg-surface p-5">
        <h2 className="text-[13px] font-semibold text-ink">Grant enterprise status</h2>
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-4">
          <input
            ...
            placeholder="Team ID"
            className="h-9 rounded-lg border border-hairline bg-void px-2.5 text-[12.5px] text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none sm:col-span-2"
          />
          <input
            type="number"
            value={grantMembers}
            ...
            className="h-9 rounded-lg border border-hairline bg-void px-2.5 text-[12.5px] text-ink placeholder:text-ink-faint focus:outline-none"
          />
          <input
            type="number"
            value={grantWebinars}
        ...
        <div className="mt-2 flex items-center gap-2">
          <input
            ...
            className="h-9 w-40 rounded-lg border border-hairline bg-void px-2.5 text-[12.5px] text-ink placeholder:text-ink-faint focus:outline-none"
          />
          <button
            onClick={() => void grant()}
      ...
      <section>
        <h2 className="text-[13px] font-semibold text-ink">Enterprise accounts</h2>
        {accounts.length === 0 ? (
