    ...
    <div className={cn("flex items-start justify-center gap-3 sm:gap-4", className)}>
      {[
        ...
        <div key={unit.label} className="flex items-start gap-3 sm:gap-4">
          {index > 0 && (
            <span className="pt-3 text-3xl font-light text-accent/40 sm:pt-5 sm:text-5xl">
              :
            </span>
          )}
          <div className="w-[88px] rounded-2xl border border-hairline bg-surface/80 px-2 py-4 backdrop-blur-xl sm:w-[124px] sm:py-6">
            <div className="text-center text-4xl font-semibold tabular-nums tracking-[-0.04em] sm:text-6xl">
              {String(unit.value).padStart(2, "0")}
            </div>
            <div className="mt-2 text-center text-[9px] font-semibold uppercase tracking-[0.22em] text-ink-muted sm:text-[10px]">
              {unit.label}
            </div>
          </div>
        </div>
