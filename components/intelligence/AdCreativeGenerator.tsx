    ...
    <div className="flex flex-col rounded-xl border border-hairline bg-surface p-4">
      ...
      {editing ? (
        ...
        <button onClick={() => setEditing(true)} className="flex-1 text-left">
          <p className="text-[14px] font-semibold text-ink">{creative.headline}</p>
          <p className="mt-1.5 text-[12.5px] leading-relaxed text-[#C8C8D4]">{creative.primary_text}</p>
          <span className="mt-2 inline-block rounded-md bg-accent/15 px-2 py-1 text-[11px] font-semibold text-accent">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        danger ? "text-ink-faint hover:bg-[#FF3B3B]/10 hover:text-[#FF3B3B]" : "text-ink-faint hover:bg-surface-2 hover:text-ink"
      )}
    >
      {children}
    </button>
